...
export const LoginSection: React.FC<LoginSectionProps> = ({ selectedRole }) => {
  ...
  <h2 className="text-2xl lg:text-3xl font-extrabold mb-2 text-center text-black dark:text-white tracking-wide">{t('dashboard.schoolName')}</h2>
  <h3 className="text-base lg:text-lg font-semibold mb-8 sm:mb-10 lg:mb-12 text-center w-full text-[#a02c2c] dark:text-red-400">{t('dashboard.fullName')}</h3>
  ...
  <p className="text-xs text-gray-700 dark:text-gray-300 mt-8 sm:mt-10 lg:mt-12 text-center px-4">
          © 2025 {t('dashboard.schoolName')} {t('header.schoolManagement')} - Developed by Codegisoft
        </p>
      </div>
    </div>
  );
