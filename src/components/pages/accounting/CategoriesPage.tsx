...
const CategoriesPage: React.FC = () => {
  ...
  const load = async () => {
    const r = await fetch('/api/accounting/categories');
    if (!r.ok) return;
  ...
  const submit = async () => {
    const r = await fetch('/api/accounting/categories', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) });
