...
const AccountingOverview: React.FC = () => {
  ...
  const fetchSummary = async () => {
    try {
      ...
      if (!summaryRes.ok) {
        await handleFetchError(summaryRes);
        return;
      }
      if (!transactionsRes.ok) {
        await handleFetchError(transactionsRes);
        return;
      }
      ...
      setError(err instanceof Error ? err.message : 'Error loading accounting data');
      showToast(err instanceof Error ? err.message : 'Failed to load accounting data', 'error');
    } finally {
      setLoading(false);
    }
  };
