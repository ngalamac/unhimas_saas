  ...
  setLoading(true);
    ...
    ...(currentBranch && { branch: (currentBranch as any)._id || (currentBranch as any).id })
      ...
      if (!res.ok) {
        ...
        let body: any = null;
        try { body = await res.clone().json(); } catch (e) { body = await res.clone().text().catch(() => null); }
        const msg = (body && (body.message || body.error)) ? (body.message || body.error) : `Request failed (${res.status})`;
        setError(msg);
        showToast(msg, 'error');
        setStudents([]);
        return;
